document.addEventListener("DOMContentLoaded", () => {
  ...
  const constructionSliderElement = document.querySelector(
    ".construction__slider",
  );
  if (constructionSliderElement) {
    const constructionIntroColumn = document.querySelector(
      ".construction__intro-column",
    );
    const constructionWrapper =
      constructionSliderElement.querySelector(".swiper-wrapper");
    const constructionIntroAnchor =
      document.createComment("construction intro");
    ...
    const syncConstructionIntro = () => {
      if (constructionMedia.matches) {
        constructionIntroColumn.classList.add(
          "construction__slide",
          "swiper-slide",
        );
      ...
      constructionIntroColumn.classList.remove(
        "construction__slide",
        "swiper-slide",
      );
    ...
    const constructionSwiper = new Swiper(constructionSliderElement, {
      ...
      spaceBetween: 16,
      navigation: {
        prevEl: "[data-construction-prev]",
        nextEl: "[data-construction-next]",
      },
      breakpoints: {
  ...
  if (floorsContainer) {
    ...
    const setFloor = (button) => {
      ...
      document
        .querySelector(".apartments__mobile-controls")
        .style.setProperty(
          "--mobile-floor-top",
          mobileFloorTop[button.dataset.floor],
        );
      document.querySelector("[data-apartments-floor]").textContent =
        button.dataset.floor;
      document.querySelector("[data-apartments-floor-mobile]").textContent =
        button.dataset.floor;
      ...
      floors.forEach((floor) => {
        floor.classList.toggle("is-active", floor === button);
        floor.setAttribute("aria-pressed", floor === button);
      });
    ...
    document
      .querySelector("[data-apartments-close]")
      .addEventListener("click", closeApartmentPanel);
    document.addEventListener("keydown", (event) => {
  ...
  if (contactForm) {
    ...
    const validateField = (input) => {
      ...
      const valid = isName
        ? /^[А-Яа-яЁёA-Za-z -]{2,}$/.test(value)
        : input.value.replace(/\D/g, "").length >= 11;
      const field = input.parentElement;
      field.classList.toggle("is-error", !valid);
      field.querySelector(".contacts__error").textContent = isName
        ? value
          ? "Не подходящее имя"
          : "Введите имя"
  ...
  const checkShowcase = () => {
    if (showcase.getBoundingClientRect().bottom <= innerHeight * 0.82)
      runShowcase();
  };
